...
def process_inpatient_periods(
    ...
    Returns:
        dict: Словарь с обновленными периодами:
            - PMP: List[PeriodType] - обновленные периоды ПМП
            - GSS: List[PeriodType] - обновленные периоды ГСС
    """
    ...
    sorted_inpatient = sorted(periods_inpatient, key=lambda x: x.DN) if periods_inpatient else []
