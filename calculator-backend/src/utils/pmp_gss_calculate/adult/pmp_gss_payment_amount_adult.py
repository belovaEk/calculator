    ...
    for l in range(len(pmp_periods)):
        ...
        for j in range(len(pmp_periods[l])):
            ...
            if j == 0:
                ...
                if omo_pmp[l].is_payment_transferred:
                    if (
                        omo_pmp[l].is_get_PSD_FSD_last_mounth_payment_trasferred
                        and omo_pmp[l].is_get_PSD_FSD_last_year_payment_trasferred
                    ):
                        if omo_pmp[l].is_Not_get_PSD_FSD_now_payment_trasferred:
            ...
            for period in omo_pmp[l].periods:
        ...
        for j in range(len(gss_periods[l])):
            ...
            # Поиск подходящего sp_standart периода
            for period in omo_gss[l].periods:
                if period.DN <= data_poiska_pensii < period.DK:
                    sp_amount = round(period.amount, 2)
                    break
