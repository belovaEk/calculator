...
async def pmp_gss_index (pmp_periods: GssPmpPensionType, gss_periods: GssPmpPensionType):
    ...
    new_pmp_periods = recalculation(pensions=pmp_periods)
    new_gss_periods = recalculation(pensions=gss_periods) 
...
def recalculation(pensions: GssPmpPensionType) -> GssPmpIndexType:
